...
| [The MCP server entry point](#the-mcp-server-entry-point) | Why the 73-tool FastMCP server is launched out-of-band (not a subcommand) and where to find its tool surface and request flow. |
...
> The session drives the same autonomous **Trinity Loop** over the 73 MCP tools and, when
...
> 73-tool surface — see [tool list](../04-mcp-tools/tool-list.md).)*
>
...
The 73-tool FastMCP server is **not** a subcommand of this Typer app. It is a
...
> The MCP tool count is **73** distinct tool functions (cite
...
- [Canonical facts](canonical-facts.md) — the numeric source of truth for the 73-tool surface and test counts cited above.
