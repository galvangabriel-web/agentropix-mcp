...
- **FastMCP** — the single Model Context Protocol (MCP) server process that exposes the
  **73** forensic tools to MCP clients over a bearer token. Code: `mcp_server/` →
...
flowchart TB
    ...
    subgraph Entry["Entry points — tailnet-only (ADR-017)"]
        ...
        MCP["mcp_server<br/>FastMCP · 73 tools<br/>bearer-token"]:::api
    end
...
|------------------|----------------|--------|
| `mcp_server/` | FastMCP server, 73 tools, Thymus, trace, config | [mcp-server.md](mcp-server.md) |
