...
| `agentropix-sift-mcp` | `agentropix_sift.mcp_server.fastmcp_app:main` | FastMCP server hosting the 73 MCP tools |
...
graph TD
    ...
    MCP["mcp_server/<br/>FastMCP server (73 tools)"]
...
The **73** distinct tool functions are reached as 73 `@app.tool()` decorator occurrences
(68 in `fastmcp_app.py` + 5 wazuh wrappers, with `wazuh_hunt_ioc` registered in two modules);
