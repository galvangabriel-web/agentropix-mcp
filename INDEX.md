...
| Project Structure | all | Where does each file/folder live and what does it do? | [README — Project Structure](README.md#project-structure) |
...
| ROCBA — Agent-Execution-Logs audit (live-MCP run) | judge, auditor | Where are the timestamped agent/tool execution logs for a real run? The ROCBA insider-IP-theft triage (`INC-2026-0613202023`) — 31 MCP requests, 2078s tool runtime, server HTTP audit + thymus-access logs, the `evidence_register` SHA-256 `f2eb856d` ground-truth match, the RDP-brute-force DRAFT finding (`indexed:false` = cannot self-approve), and the honest negatives kept on record (carve param-bug re-run, `report_generate` DRAFT-only gotcha, memory-init timeout). A sibling [WINXP-LAPTOP-2005](case-activation/runs/WINXP-LAPTOP-2005/WINXP-LAPTOP-2005-execution-chain.md) execution-chain run is also published. | [case-activation/runs/rocba/EXECUTION-LOG.md](case-activation/runs/rocba/EXECUTION-LOG.md) |
...
Sealed DFIR case reports — the investigative narratives behind the runs ([section README](docs/12-CASES-REPORTS/README.md)): [SRL-2018 forensic report](docs/12-CASES-REPORTS/srl-2018-report/SRL-2018-FORENSIC-REPORT.md) (exec summary · ATT&CK lifecycle · timeline · IOCs · 10 sealed findings, with [technical appendix](docs/12-CASES-REPORTS/srl-2018-report/TECHNICAL-APPENDIX.md) and [Wazuh IOC gallery](docs/12-CASES-REPORTS/srl-2018-report/WAZUH-IOC-GALLERY.md)) · [SRL-2015 report](docs/12-CASES-REPORTS/srl-2015-report/README.md) · [Vanko report](docs/12-CASES-REPORTS/vanko-report/README.md) · [CFReDS "Hacking Case" report](docs/12-CASES-REPORTS/cfreds-hacking-case-report/README.md) (single-host Windows XP disk case — Greg Schardt / "Mr. Evil" unauthorized wireless interception; no memory image, no enterprise lateral movement; 35 examiner-approved findings · 24 timeline events · 93 IOCs staged, with [executive summary](docs/12-CASES-REPORTS/cfreds-hacking-case-report/CFREDS-executive.md), [analyst report](docs/12-CASES-REPORTS/cfreds-hacking-case-report/CFREDS-analyst.md), [attack execution graph](docs/12-CASES-REPORTS/cfreds-hacking-case-report/diagrams/attack-graph.png) and [agent execution log](docs/12-CASES-REPORTS/cfreds-hacking-case-report/audit/PROJECT-agent-execution-log.md)) · [SRL-2018 artifact inventory](docs/12-CASES-REPORTS/srl-2018-artifact-inventory.md) (9,578 findings across 29 hosts — IPs, hashes, YARA hits, technique matrix).
