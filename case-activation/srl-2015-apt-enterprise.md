...
Both lanes drive the **same deterministic 71-tool MCP engine** and reach the same
...
1. **Pre-flight the environment.**
   ...
   **Expect:** `All tools available.` and `health` → `status:"ok"` with a live `tool_count` (canonical **71**; trust the live number).
