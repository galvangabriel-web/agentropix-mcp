...
> ```
> Then call the `health` tool — expect `{ "status":"ok", "tool_count":71, ... }`. **Trust the live
> `health.tool_count`, not the startup banner** (canonical `71`; a live server may report `72` — the
> `wazuh_hunt_ioc` double-registration; see the user guide §1.2).
...
1. **Verify the MCP is up.**
   ...
   **Expect:** `✓ Connected`; `health` → `status:"ok"`, live `tool_count` (canonical `71`; trust the live number).
